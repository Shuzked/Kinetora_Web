...
const CaseStudyPost = () => {
  ...
  React.useEffect(() => {
    ...
    const tick = () => {
      // Calcular inicio y fin del rango de scroll
      const start = gridEl.getBoundingClientRect().top + window.scrollY - window.innerHeight * 0.2;
      const end = readyEl.getBoundingClientRect().top + window.scrollY - 108; // offset aprox. del navbar
      const dist = Math.max(1, end - start);
      const y = window.scrollY;
      const p = clamp((y - start) / dist, 0, 1);
      const t = easeOutCubic(p);

      // Desplazamientos iniciales para dar sensación de velocidades distintas
      const leftInit = 80;   // texto
      const rightInit = 140; // media
    ...
    // Asegurar will-change para suavidad
