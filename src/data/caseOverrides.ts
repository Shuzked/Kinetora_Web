...
> = {
  ...
  "cybertitans-clash-impacto-brutal-y-evolucion-web3": {
    esTextHtml: [
      ...
      "<p>Nos encargamos de todo el control creativo de esta actualización. Diseñamos un entorno visual agresivo y tecnológico, entregamos la web oficial de la campaña y creamos el arte interno del juego y los anuncios visuales de las notas del parche. El impacto fue masivo: superamos los 1,5 millones de visitas orgánicas y la actualización mantuvo a los jugadores muy activos dentro del ecosistema.</p>",
      "<p>Nuestra agencia construyó toda la plataforma visual. A continuación, detallamos cada entregable enviado a la desarrolladora.</p>",
      "<h2>Estrategia de Marketing Web3: Activos Creados</h2>",
      "<p>Rompimos las reglas clásicas del diseño digital para crear un paquete visual directo y muy efectivo. Nos centramos solo en tres entregables clave:</p>",
      "<h3>1. Landing Page y entorno web interactivo</h3>",
      "<p>Desarrollamos una landing page espectacular. El nuevo logotipo y su icono de fuego son los protagonistas del encabezado. Usamos tonos naranjas y amarillos muy vivos sobre fondos morados y azules oscuros.</p>",
      "<p>Destacamos los mensajes clave del nuevo modo: las partidas rápidas de 3 jugadores y el sistema de multiplicadores de premios. La web cargaba rápido y llevaba al usuario directo a jugar. Fue el motor de nuestro Marketing Web3.</p>",
      "<h3>2. Banners e imágenes internas del juego</h3>",
      "<p>La emoción de la actualización debe sentirse nada más abrir el juego. Por eso, adaptamos las superficies visuales internas de CyberTitans.</p>",
      "<p>Diseñamos banners estáticos de gran impacto e imágenes exclusivas para los menús principales. Este sistema visual mantuvo a la comunidad alerta e interactuando desde el primer segundo. Todo se integró perfectamente para potenciar la participación en los nuevos retos y eventos.</p>",
      "<h3>3. Notas del parche visuales y anuncios</h3>",
      "<p>Informar a los jugadores exige claridad y estilo. Diseñamos todos los anuncios gráficos para las notas del parche (patch notes).</p>",
      "<p>Creamos visuales potentes para acompañar las novedades de los desarrolladores. Explicamos las nuevas mecánicas de forma directa y muy visual. Estos activos educaron a toda la comunidad rápidamente. Así logramos que el modo Clash se entendiera al instante y tuviera una gran viralidad orgánica.</p>",
    ...
    enTextHtml: [
      "<p>Updating a video game requires a strong visual impact. Introducing a new game mode is a significant challenge—one that demands a solid Web3 Marketing strategy. LitLab Games once again trusted our agency to launch their latest update. The \"Clash\" mode was born to revolutionize gameplay and keep the world in constant evolution.</p>",
      "<p>Our primary goal was crystal clear: generate massive anticipation before the official launch and keep the entire player community engaged.</p>",
      "<p>We took full creative control of this update. We designed an aggressive, high‑tech visual environment, delivered the official campaign website, and created the in‑game assets and visual announcements for the patch notes. The impact was massive: we surpassed 1.5 million organic visits, and the update kept players highly active within the ecosystem.</p>",
      "<p>Our agency built the entire visual platform. Below, we detail every deliverable sent to the developer:</p>",
      "<h2>Web3 Marketing Strategy: Created Assets</h2>",
      "<p>We broke the classic rules of digital design to create a direct and highly effective visual package. We focused strictly on these three key deliverables:</p>",
      "<h3>1. Landing Page and Interactive Web Environment</h3>",
      "<p>We developed a spectacular landing page. The new logo and its fire icon take center stage in the header. We used vibrant orange and yellow tones against deep purple and dark blue backgrounds.</p>",
      "<p>We highlighted key messages of the new mode: the fast‑paced 3‑player matches and the prize multiplier system. The site loaded fast and guided the user straight to the game—the engine of our Web3 Marketing.</p>",
      "<h3>2. In‑Game Banners and Internal Assets</h3>",
      "<p>The excitement of the update must be felt as soon as the game opens. Therefore, we adapted the internal visual surfaces of CyberTitans.</p>",
      "<p>We designed high‑impact static banners and exclusive images for the main menus. This visual system kept the community alert and engaged from the first second. Everything was perfectly integrated to boost participation in the new challenges and events.</p>",
      "<h3>3. Visual Patch Notes and Announcements</h3>",
      "<p>Informing players demands clarity and style. We designed all graphical announcements for the patch notes.</p>",
      "<p>We created powerful visuals to accompany the developers' news, explaining new mechanics directly and visually. These assets quickly educated the entire community—ensuring the Clash mode was instantly understood and achieved great organic virality.</p>",
      "<h2>The Impact of a Great Visual Campaign</h2>",
      "<p>This update was an absolute victory for LitLab Games. Mobilizing thousands of players demonstrates the power of Web3 Marketing. A strong graphical image always improves real user retention. Thanks to this, the game strengthened its clear leadership in the market.</p>",
    ].join(""),
    enMediaHtml: [
