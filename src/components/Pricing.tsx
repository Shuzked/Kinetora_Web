...
const Pricing = () => {
  return (
    <section id="precios" className="py-24 bg-[#0D0D0D]">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan, i) => (
            <motion.div 
              key={i}
              whileHover={{ y: -5 }}
              className={`relative p-8 rounded-[2rem] border ${plan.featured ? 'border-[#B454FF] bg-[#B454FF]/5 shadow-[0_0_40px_rgba(180,84,255,0.1)]' : 'border-[#2A2A2A] bg-[#111111]'}`}
            >
              {plan.featured && (
              ...
              <div className="text-4xl font-black text-[#F5F5F5] mb-4">{plan.price}<span className="text-sm text-[#2A2A2A] font-normal">/mes</span></div>
              ...
              <Button className={`w-full rounded-full h-12 font-bold text-xs tracking-widest ${plan.featured ? 'bg-[#B454FF] hover:bg-[#B454FF]/90 text-white' : 'bg-[#2A2A2A] hover:bg-[#2A2A2A]/80 text-[#F5F5F5]'}`}>
                EMPEZAR AHORA
              </Button>
            </motion.div>
          ))}
